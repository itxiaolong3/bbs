...
//通过get方式获取贴文类型标志
$gettype=isset($_GET['fortype']) ? $_GET['fortype'] : 0;
...
if($gettype==0){
    ...
    $sqlforlimit="select count(*) as sum from publish where pub_module_id=$gettype";
}
...
if($gettype==0){
    ...
    $sql="select *from publish  LEFT JOIN user on pub_owner=user_name where pub_module_id=$gettype order by pub_time desc limit $offset,$rowsPerpage";
}
